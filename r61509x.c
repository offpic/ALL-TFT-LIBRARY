...
/*!
	...
	@brief          Based on Chan's MCI_OLED@LPC23xx-demo thanks!				@n
					Available TFT-LCM are listed below.							@n
					 ...
					 -S95361					(ILI9326)   8/16bit mode.		@n
					 -BY-T28WB05P3-V05			(OTM4001A)  8/16bit mode.

    @section HISTORY
		2012.04.29	V1.00	Revised from r61509v.c
		2014.10.15	V2.00	Fixed 8-bit access bug.
		2014.11.01	V3.00	Added OTM4001A Support.
...
inline void R61509x_wr_cmd(uint16_t cmd)
{
	...
	R61509x_CMD = (uint8_t)(cmd>>8);		/* upper 8bit command	*/
	R61509x_WR();							/* WR=L->H				*/
	R61509x_CMD = (uint8_t)cmd;				/* lower 8bit command	*/
#else
	R61509x_CMD = cmd;						/* 16bit command		*/
#endif
	R61509x_WR();							/* WR=L->H				*/
...
void R61509x_init(void)
{
	...
	{
		/* Initialize SPFD5420A and OTM4001A */
		R61509x_wr_cmd(0x0606);
		R61509x_wr_dat(0x0000);
		_delay_ms(1);
