...
/*!
    ...
    @section HISTORY
		...
		2011.01.20  V4.00   Added Confirmation RDID1
...
*/
/**************************************************************************/
inline uint8_t ST7735_rd_cmd(uint8_t cmd)
{
	uint8_t val;
...
inline uint8_t ST7735_rd_cmd(uint8_t cmd)
{
#ifdef ST7735_SPI_4WIRE_READID_IGNORE
 #warning "Ingnore ST7735 RDID1 check!"
	return 0x5C;
#else	
 #warning "ST7735 USES SDA(Input&Output Multiplexed) Line!"
 	uint8_t temp;
...
void ST7735_init(void)
{
	...
	if((devicetype == 0x5C) || (devicetype == 0x7C) || (devicetype == 0xE3))
	{
		/* Initialize ST7735 */
